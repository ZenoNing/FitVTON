...
# GarmentCode VTON post-step constraints (invoked from garment.py after XPBD substeps).


@wp.kernel
def apply_waistband_side_constraint_kernel(
    particle_q: wp.array(dtype=wp.vec3),
    particle_qd: wp.array(dtype=wp.vec3),
    upper_indices: wp.array(dtype=wp.int32),
    lower_indices: wp.array(dtype=wp.int32),
    lower_count: int,
    mode: int,
    margin: float,
    stiffness: float,
    ...
    tuck_depth: float,
    damping: float,
    lower_y_window: float,
):
    tid = wp.tid()
    idx = upper_indices[tid]

    center_x = float(0.0)
    center_z = float(0.0)
    for i in range(lower_count):
        lower_point = particle_q[lower_indices[i]]
        center_x += lower_point[0]
        center_z += lower_point[2]
    inv_lower_count = float(1.0) / float(lower_count)
    center_x *= inv_lower_count
    center_z *= inv_lower_count

    point = particle_q[idx]
    point_x = point[0]
    point_y = point[1]
    point_z = point[2]

    nearest_id = int(0)
    nearest_dist_sq = float(1.0e20)
    nearest_window_id = int(0)
    nearest_window_dist_sq = float(1.0e20)
    has_window_match = bool(False)
    for i in range(lower_count):
        lower_point = particle_q[lower_indices[i]]
        dx = lower_point[0] - point_x
        dz = lower_point[2] - point_z
        dist_sq = dx * dx + dz * dz
        if dist_sq < nearest_dist_sq:
            nearest_dist_sq = dist_sq
            nearest_id = i
        dy_abs = wp.abs(lower_point[1] - point_y)
        if lower_y_window <= 0.0 or dy_abs <= lower_y_window:
            if dist_sq < nearest_window_dist_sq:
                nearest_window_dist_sq = dist_sq
                nearest_window_id = i
                has_window_match = True

    if has_window_match:
        nearest_id = nearest_window_id
    nearest_lower = particle_q[lower_indices[nearest_id]]
    lower_dx = nearest_lower[0] - center_x
    lower_dz = nearest_lower[2] - center_z
    boundary_radius = wp.sqrt(lower_dx * lower_dx + lower_dz * lower_dz)
    if boundary_radius <= 1.0e-6:
        return

    dx = point_x - center_x
    dz = point_z - center_z
    radius = wp.sqrt(dx * dx + dz * dz)
    dir_x = float(0.0)
    dir_z = float(0.0)
    if radius > 1.0e-6:
        dir_x = dx / radius
        ...
        dir_norm = wp.sqrt(lower_dx * lower_dx + lower_dz * lower_dz)
        if dir_norm <= 1.0e-6:
            return
        dir_x = lower_dx / dir_norm
        dir_z = lower_dz / dir_norm
        radius = float(0.0)

    target_radius = float(0.0)
    radius_needs_update = bool(False)
    if mode == 1:
        target_radius = boundary_radius - margin
        if target_radius < 0.0:
            target_radius = 0.0
        radius_needs_update = radius > target_radius
    else:
        target_radius = boundary_radius + margin
        radius_needs_update = radius < target_radius

    changed = bool(False)
    if radius_needs_update:
        new_radius = (1.0 - stiffness) * radius + stiffness * target_radius
        point_x = center_x + dir_x * new_radius
        point_z = center_z + dir_z * new_radius
        changed = True

    if mode == 1:
        target_y = nearest_lower[1] - tuck_depth
        if point_y > target_y:
            point_y = (1.0 - vertical_stiffness) * point_y + vertical_stiffness * target_y
            changed = True

    if changed:
        particle_q[idx] = wp.vec3(point_x, point_y, point_z)
        particle_qd[idx] = particle_qd[idx] * (1.0 - damping)


@wp.kernel
def apply_lower_body_clearance_constraint_kernel(
    particle_q: wp.array(dtype=wp.vec3),
    particle_qd: wp.array(dtype=wp.vec3),
    lower_indices: wp.array(dtype=wp.int32),
    ...
    for i in range(body_count):
        body_point = body_points[body_indices[i]]
        center_x += body_point[0]
        center_z += body_point[2]
    inv_body_count = float(1.0) / float(body_count)
    center_x *= inv_body_count
    center_z *= inv_body_count
    ...
    fallback_body_radius = float(0.0)
    has_window_match = bool(False)
    for i in range(body_count):
        body_point = body_points[body_indices[i]]
        body_dx = body_point[0] - center_x
        body_dz = body_point[2] - center_z
        projection = body_dx * dir_x + body_dz * dir_z
        if projection > fallback_body_radius:
            fallback_body_radius = projection
        dy_abs = wp.abs(body_point[1] - point[1])
        if body_y_window <= 0.0 or dy_abs <= body_y_window:
            has_window_match = True
            if projection > body_radius:
                body_radius = projection

    if not has_window_match:
        body_radius = fallback_body_radius

    target_radius = body_radius + margin
    if radius >= target_radius:
        return

    step = (target_radius - radius) * stiffness
    ...
    particle_q[idx] = wp.vec3(center_x + dir_x * new_radius, point[1], center_z + dir_z * new_radius)
    particle_qd[idx] = particle_qd[idx] * (1.0 - damping)


@wp.kernel
def apply_untucked_waistband_fit_constraint_kernel(
    particle_q: wp.array(dtype=wp.vec3),
    particle_qd: wp.array(dtype=wp.vec3),
    lower_indices: wp.array(dtype=wp.int32),
    body_points: wp.array(dtype=wp.vec3),
    ...
    for i in range(body_count):
        body_point = body_points[body_indices[i]]
        dy_abs = wp.abs(body_point[1] - point[1])
        if body_y_window > 0.0 and dy_abs > body_y_window:
            continue
        dx = point[0] - body_point[0]
        dz = point[2] - body_point[2]
        dist_sq = dx * dx + dz * dz
        if dist_sq < best_dist_sq:
            best_dist_sq = dist_sq
            best_body_x = body_point[0]
            best_body_z = body_point[2]
            found = True

    if not found:
        return

    off_x = point[0] - best_body_x
    off_z = point[2] - best_body_z
    dist = wp.sqrt(off_x * off_x + off_z * off_z)
    if dist <= margin + 1.0e-6:
        return

    inv_dist = float(1.0) / dist
    target_x = best_body_x + off_x * inv_dist * margin
    target_z = best_body_z + off_z * inv_dist * margin

    move_x = (target_x - point[0]) * stiffness
    move_z = (target_z - point[2]) * stiffness
    move_len = wp.sqrt(move_x * move_x + move_z * move_z)
    if move_len <= 1.0e-8:
        return
    if max_step > 0.0 and move_len > max_step:
        scale = max_step / move_len
        move_x *= scale
        move_z *= scale

    particle_q[idx] = wp.vec3(point[0] + move_x, point[1], point[2] + move_z)
    particle_qd[idx] = particle_qd[idx] * (1.0 - damping)


@wp.kernel
def apply_limb_end_stop_constraint_kernel(
    particle_q: wp.array(dtype=wp.vec3),
    particle_qd: wp.array(dtype=wp.vec3),
    cloth_indices: wp.array(dtype=wp.int32),
    body_points: wp.array(dtype=wp.vec3),
    proximal_indices: wp.array(dtype=wp.int32),
    distal_indices: wp.array(dtype=wp.int32),
    proximal_count: int,
    distal_count: int,
    stiffness: float,
    damping: float,
    ...
    if proximal_count <= 0 or distal_count <= 0:
        return

    proximal_center = wp.vec3(0.0, 0.0, 0.0)
    for i in range(proximal_count):
        proximal_center += body_points[proximal_indices[i]]
    proximal_center = proximal_center / float(proximal_count)

    distal_center = wp.vec3(0.0, 0.0, 0.0)
    for i in range(distal_count):
        distal_center += body_points[distal_indices[i]]
    distal_center = distal_center / float(distal_count)
    ...
    proximal_max = float(-1.0e20)
    for i in range(proximal_count):
        body_point = body_points[proximal_indices[i]]
        projection = wp.dot(body_point - proximal_center, axis)
        if projection > proximal_max:
            proximal_max = projection

    distal_min = float(1.0e20)
    for i in range(distal_count):
        body_point = body_points[distal_indices[i]]
        projection = wp.dot(body_point - proximal_center, axis)
        if projection < distal_min:
            distal_min = projection

    limit = (proximal_max + distal_min) * 0.5 - margin
    idx = cloth_indices[tid]
    point = particle_q[idx]
    point_projection = wp.dot(point - proximal_center, axis)
    overshoot = point_projection - limit
    if overshoot <= 0.0:
        return

    step = overshoot * stiffness
    if max_step > 0.0 and step > max_step:
        step = max_step
    if step <= 0.0:
        return

    particle_q[idx] = point - axis * step
    particle_qd[idx] = particle_qd[idx] * (1.0 - damping)
...
def update_body_pose(
    ...
    tid = wp.tid()
    mesh = wp.mesh_get(shape)
    mesh.velocities[tid] = (vertices[tid] - mesh.points[tid]) / frame_dt
    mesh.points[tid] = vertices[tid]
