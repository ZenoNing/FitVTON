...
def _np_array_from_data(data, dtype=None, ndmin=0):
    """Convert input to ndarray; fall back to a copy when NumPy 2.x rejects copy=False."""
    try:
        return np.array(data, dtype=dtype, copy=False, ndmin=ndmin)
    except ValueError:
        return np.array(data, dtype=dtype, copy=True, ndmin=ndmin)
...
class array(Array):
    ...
    def _init_from_data(self, data, dtype, shape, device, copy, pinned):
        ...
        if dtype == Any:
            ...
            try:
                arr = _np_array_from_data(data, ndmin=1)
            ...
            try:
                arr = _np_array_from_data(data, dtype=npdtype, ndmin=1)
            except Exception as e:
