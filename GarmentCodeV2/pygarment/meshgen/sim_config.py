...
class PathCofig:
    ...
    def render_frame_path(self, frame_idx: int, camera_name: str = ''):
        """Render output path for a specific simulation frame.

        Keeps the existing naming scheme but adds a frame token so that
        per-frame renders do not overwrite each other.
        """
        fname = (
            f'{self.sim_tag}_frame_{frame_idx:05d}_render_{camera_name}.png'
            if camera_name
            else f'{self.sim_tag}_frame_{frame_idx:05d}_render.png'
        )
        return self.out_el / 'frames' / fname
...
class PathCofigNew:
    ...
    def __init__(self, 
        ...
        self.in_tag = in_name
        # self.out_folder_tag = f'{out_name}_{datetime.now().strftime("%y%m%d-%H-%M-%S")}' if add_timestamp else out_name
        self.out_folder_tag = out_name
        garment_name = Path(out_path).name
        if out_name == 'Ref':
            self.sim_tag = f'{in_name}' # For reference image
            self.boxmesh_tag = f'{in_name}' # For reference image
        else:
            self.sim_tag = f'{garment_name}_{out_name}_{self._body_name}' 
            self.boxmesh_tag = f'{garment_name}_{out_name}_{self._body_name}' 
        ...
        self.out = out_path
        if out_name == 'Ref':
            self.out_el = Path(out_path) # For reference image
        else:
            self.out_el = Path(out_path) / self.out_folder_tag / self._body_name
    ...
    def _update_in_paths(self):
        ...
        # self.in_body_obj = self.bodies_path / f'{self._body_name}.obj'
        self.in_g_spec = self.input / f'{self.in_tag}_specification.json'
        self.body_seg = Path(self._system['bodies_default_path']) / ('ggg_body_segmentation.json' if not self.use_smpl_seg else 'smplx_body_segmentation.json')
        # self.in_design_params = self.input / f'{self.in_tag}_design_params.yaml'
        self.in_design_params = self.input / 'design_params.yaml'
...
class SimConfig:
    def __init__(self, sim_props):
        ...
        self.post_dynamic_frames = int(self.get_sim_props_value(sim_props, 'post_dynamic_frames', 100))
        # Quality filter
        ...
        # Tucked / untucked side constraint between an upper hem and lower waistband.
        self.enable_waistband_side_constraint = self.get_sim_props_value(
            sim_props_option,
            'enable_waistband_side_constraint',
            False)
        self.waistband_label = self.get_sim_props_value(
            sim_props_option,
            'waistband_label',
            'lower_interface')
        self.upper_hem_label = self.get_sim_props_value(
            sim_props_option,
            'upper_hem_label',
            'upper_hem')
        self.waistband_side_margin = self.get_sim_props_value(
            sim_props_option,
            'waistband_side_margin',
            1.0)
        self.waistband_constraint_stiffness = self.get_sim_props_value(
            sim_props_option,
            'waistband_constraint_stiffness',
            0.25)
        self.waistband_constraint_damping = self.get_sim_props_value(
            sim_props_option,
            'waistband_constraint_damping',
            0.3)
        self.waistband_vertical_stiffness = self.get_sim_props_value(
            sim_props_option,
            'waistband_vertical_stiffness',
            0.35)
        self.waistband_tuck_depth = self.get_sim_props_value(
            sim_props_option,
            'waistband_tuck_depth',
            4.0)
        self.upper_tuck_band_height = self.get_sim_props_value(
            sim_props_option,
            'upper_tuck_band_height',
            8.0)
        self.waistband_constraint_frames = self.get_sim_props_value(
            sim_props_option,
            'waistband_constraint_frames',
            400)
        self.waistband_side_lower_scope = self.get_sim_props_value(
            sim_props_option,
            'waistband_side_lower_scope',
            'waistband')
        self.waistband_side_lower_y_window = self.get_sim_props_value(
            sim_props_option,
            'waistband_side_lower_y_window',
            8.0)
        self.waistband_overlap_margin = self.get_sim_props_value(
            sim_props_option,
            'waistband_overlap_margin',
            1.0)
        self.waistband_overlap_band_margin = self.get_sim_props_value(
            sim_props_option,
            'waistband_overlap_band_margin',
            1.0)
        self.disable_untucked_upper_hem_constraint = self.get_sim_props_value(
            sim_props_option,
            'disable_untucked_upper_hem_constraint',
            False)
        self.enable_untucked_waistband_fit_constraint = self.get_sim_props_value(
            sim_props_option,
            'enable_untucked_waistband_fit_constraint',
            False)
        self.untucked_waistband_fit_scope = self.get_sim_props_value(
            sim_props_option,
            'untucked_waistband_fit_scope',
            'waistband')
        self.untucked_waistband_fit_band_height = self.get_sim_props_value(
            sim_props_option,
            'untucked_waistband_fit_band_height',
            6.0)
        self.untucked_waistband_fit_margin = self.get_sim_props_value(
            sim_props_option,
            'untucked_waistband_fit_margin',
            0.35)
        self.untucked_waistband_fit_stiffness = self.get_sim_props_value(
            sim_props_option,
            'untucked_waistband_fit_stiffness',
            0.22)
        self.untucked_waistband_fit_damping = self.get_sim_props_value(
            sim_props_option,
            'untucked_waistband_fit_damping',
            0.2)
        self.untucked_waistband_fit_max_step = self.get_sim_props_value(
            sim_props_option,
            'untucked_waistband_fit_max_step',
            0.5)
        self.untucked_waistband_fit_frames = self.get_sim_props_value(
            sim_props_option,
            'untucked_waistband_fit_frames',
            0)
        self.untucked_waistband_fit_body_y_window = self.get_sim_props_value(
            sim_props_option,
            'untucked_waistband_fit_body_y_window',
            8.0)
        self.enable_lower_body_clearance_constraint = self.get_sim_props_value(
            sim_props_option,
            'enable_lower_body_clearance_constraint',
            False)
        self.lower_body_clearance_scope = self.get_sim_props_value(
            sim_props_option,
            'lower_body_clearance_scope',
            'garment')
        self.lower_body_clearance_margin = self.get_sim_props_value(
            sim_props_option,
            'lower_body_clearance_margin',
            1.0)
        self.lower_body_clearance_stiffness = self.get_sim_props_value(
            sim_props_option,
            'lower_body_clearance_stiffness',
            0.08)
        self.lower_body_clearance_damping = self.get_sim_props_value(
            sim_props_option,
            'lower_body_clearance_damping',
            0.2)
        self.lower_body_clearance_max_step = self.get_sim_props_value(
            sim_props_option,
            'lower_body_clearance_max_step',
            0.25)
        self.lower_body_clearance_band_height = self.get_sim_props_value(
            sim_props_option,
            'lower_body_clearance_band_height',
            4.0)
        self.lower_body_clearance_frames = self.get_sim_props_value(
            sim_props_option,
            'lower_body_clearance_frames',
            0)
        self.lower_body_clearance_body_y_window = self.get_sim_props_value(
            sim_props_option,
            'lower_body_clearance_body_y_window',
            8.0)
        self.enable_limb_end_stop_constraint = self.get_sim_props_value(
            sim_props_option,
            'enable_limb_end_stop_constraint',
            False)
        self.limb_end_stop_stiffness = self.get_sim_props_value(
            sim_props_option,
            'limb_end_stop_stiffness',
            0.8)
        self.limb_end_stop_damping = self.get_sim_props_value(
            sim_props_option,
            'limb_end_stop_damping',
            0.2)
        self.limb_end_stop_max_step = self.get_sim_props_value(
            sim_props_option,
            'limb_end_stop_max_step',
            1.0)
        self.limb_end_stop_margin = self.get_sim_props_value(
            sim_props_option,
            'limb_end_stop_margin',
            0.0)
        self.limb_end_stop_candidate_fraction = self.get_sim_props_value(
            sim_props_option,
            'limb_end_stop_candidate_fraction',
            0.15)
