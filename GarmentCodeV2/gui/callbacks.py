...
# Available SMPL-X pose files (index 0-9 → files 00001_0.npz .. 00010_0.npz)
AVAILABLE_POSES = {i: f'Pose {i+1} ({(i+1):05d}_0.npz)' for i in range(10)}
...
# Mapping from (section, param_name) to body measurement key for cm display
# Each entry: (section, param) -> (body_key, description)
# The cm value is computed as: slider_value * body[body_key]
CM_PARAM_MAPPINGS = {
    ('shirt', 'length'): ('waist_line', 'shirt length'),
    ('shirt', 'width'): ('bust', 'bust fit'),
    ('sleeve', 'length'): ('arm_length', 'sleeve length'),
    ('cuff', 'cuff_len'): ('arm_length', 'cuff length'),
    ('waistband', 'waist'): ('waist', 'waist circ.'),
    ('waistband', 'width'): ('hips_line', 'band width'),
    ('skirt', 'length'): ('_leg_length', 'skirt length'),
    ('flare-skirt', 'length'): ('_leg_length', 'skirt length'),
    ('pencil-skirt', 'length'): ('_leg_length', 'skirt length'),
    ('levels-skirt', 'length'): ('_leg_length', 'skirt length'),
    ('pants', 'length'): ('_leg_length', 'pants length'),
    ('collar', 'fc_depth'): ('_bust_line', 'front neckline depth'),
    ('collar', 'bc_depth'): ('_bust_line', 'back neckline depth'),
}
...
class GUIState:
    ...
    def __init__(self) -> None:
        ...
        # cm label references: maps a unique key -> (section, param, ui.label, design_params_dict)
        self.ui_cm_labels = {}
        ...
        # Dynamic sim frames
        self.path_static_frames = '/sim_frames'
        self.local_path_frames = Path('./tmp_gui/sim_frames') / self.pattern_state.id
        self.local_path_frames.mkdir(parents=True, exist_ok=True)
        app.add_static_files(self.path_static_frames, str(self.local_path_frames))

        # SMPL-X parameters
        self.smplx_pose_idx = 0  # Pose index (0-9)
        self.smplx_beta_params = [0.0] * 10  # 10-dim beta
    ...
    def release(self):
        ...
        if self.local_path_frames.exists():
            shutil.rmtree(self.local_path_frames, ignore_errors=True)
    ...
    def view_tabs_layout(self):
        ...
        with ui.column(wrap=False).classes(f'h-[{self.h_params_content}vh] w-full items-center'):
            with ui.tabs() as tabs: 
                ...
                self.ui_sim_tab = ui.tab('Dynamic Sim')
            with ui.tab_panels(tabs, value=self.ui_2d_tab, animated=True).classes('w-full h-full items-center'):  
                ...
                with ui.tab_panel(self.ui_3d_tab).classes('w-full h-full items-center p-0 m-0'):
                    self.def_3d_scene()
                with ui.tab_panel(self.ui_sim_tab).classes('w-full h-full items-center p-0 m-0'):
                    self.def_dynamic_sim_tab()
    ...
    def def_body_tab(self):
        ...
        with ui.scroll_area().classes('w-full h-full p-0 m-0'):
            ...
            with ui.card().classes('w-full shadow-md border m-0 mb-2 rounded-md'):
                ui.label('SMPL-X Pose & Shape').classes('text-base font-semibold')
                
                ui.label('Target Pose').classes('p-0 m-0 mt-2 text-stone-500 text-[0.85rem]')
                self.ui_pose_select = ui.select(
                    AVAILABLE_POSES,
                    value=self.smplx_pose_idx,
                    on_change=lambda e: self._on_pose_change(e.value)
                ).classes('w-full')

                ui.label('Body Shape (Beta)').classes('p-0 m-0 mt-2 text-stone-500 text-[0.85rem]')
                ui.label('β₀ (身高/胖瘦)').classes('p-0 m-0 text-stone-400 text-[0.75rem]')
                self.ui_beta0_slider = ui.slider(
                    value=0.0, min=-2.0, max=2.0, step=0.1
                ).props('snap label').classes('w-full') \
                    .on('update:model-value',
                        lambda e: self._on_beta_change(0, e.args),
                        throttle=0.3, leading_events=False)

                ui.label('β₁ (体型比例)').classes('p-0 m-0 text-stone-400 text-[0.75rem]')
                self.ui_beta1_slider = ui.slider(
                    value=0.0, min=-2.0, max=2.0, step=0.1
                ).props('snap label').classes('w-full') \
                    .on('update:model-value',
                        lambda e: self._on_beta_change(1, e.args),
                        throttle=0.3, leading_events=False)

            ui.separator()
            ui.label('Body Measurements').classes('text-base font-semibold mt-1')
    ...
    def _on_pose_change(self, value):
        self.smplx_pose_idx = value
        print(f'INFO::SMPL-X pose changed to {value}')

    def _on_beta_change(self, idx, value):
        self.smplx_beta_params[idx] = float(value)
        print(f'INFO::SMPL-X beta[{idx}] changed to {value}')

    def _find_cm_mapping(self, section_path, param):
        """Find a cm mapping key by trying sections from innermost to outermost"""
        for section in reversed(section_path):
            key = (section, param)
            if key in CM_PARAM_MAPPINGS:
                return key
        return None

    def _compute_cm_value(self, section, param_name, value):
        """Compute the cm value for a parameter if a mapping exists"""
        key = (section, param_name)
        if key in CM_PARAM_MAPPINGS:
            body_key, _ = CM_PARAM_MAPPINGS[key]
            body_val = self.pattern_state.body_params[body_key]
            return value * body_val
        return None

    def _update_all_cm_labels(self):
        """Refresh all cm labels based on current body and design params"""
        for label_key, (section, param_name, label_elem, design_dict) in self.ui_cm_labels.items():
            val = design_dict[param_name]['v']
            cm_val = self._compute_cm_value(section, param_name, val)
            if cm_val is not None:
                label_elem.set_text(f'\u2248 {cm_val:.1f} cm')

    def def_flat_design_subtab(self, ui_elems, design_params, use_collapsible=False, section_path=()):
        ...
        for param in design_params: 
            ...
            if 'v' not in design_params[param]:
                ...
                if use_collapsible:
                    with ui.expansion().classes('w-full p-0 m-0') as expansion: 
                        ...
                        with ui.row().classes('w-full h-full p-0 m-0'):  # Ensures correct application of style classes for children
                            self.def_flat_design_subtab(ui_elems[param], design_params[param], section_path=section_path + (param,))
                else:
                ...
                if 'select' in p_type:
                    ...
                    # Check if this parameter has a cm mapping
                    cm_key = self._find_cm_mapping(section_path, param)
                    cm_val = None
                    if cm_key:
                        cm_val = self._compute_cm_value(cm_key[0], cm_key[1], val)
                    with ui.row().classes('w-full items-center p-0 m-0 gap-1'):
                        ui.label(param_name).classes('p-0 m-0 mt-2 text-stone-500 text-[0.85rem]')
                        if cm_val is not None:
                            cm_label = ui.label(f'\u2248 {cm_val:.1f} cm').classes(
                                'p-0 m-0 mt-2 text-sky-600 text-[0.8rem] ml-auto'
                            )
                            # Use unique key combining path + param to avoid collisions (e.g. left.shirt.length vs shirt.length)
                            label_key = section_path + (param,)
                            self.ui_cm_labels[label_key] = (cm_key[0], param, cm_label, design_params)
                    ui.label(param_name).classes('p-0 m-0 mt-2 text-stone-500 text-[0.85rem]')
    ...
    def def_dynamic_sim_tab(self):
        """Dynamic simulation tab with interactive 3D playback (lazy-loaded frames)"""

        with ui.row().classes('w-full p-0 m-0 justify-between items-center'):
            ui.button(
                'Run Dynamic Simulation',
                on_click=lambda: self.run_dynamic_sim()
            ).props('color=deep-purple')
            self.ui_sim_status = ui.label('Ready').classes(
                f'font-semibold text-[{theme_colors.primary}] border-[{theme_colors.primary}] '
                'border py-0 px-1.5 rounded-md'
            )

        ui.label('Dynamic Simulation').classes('text-base font-semibold')
        y_fov = np.pi / 6.
        camera_location = [0, -200., 1.25]
        camera = self.create_camera(camera_location, y_fov)
        with ui.scene(
        ...
        ).classes('w-full h-[70vh] p-0 m-0') as self.ui_sim_3d_scene:
            self.create_lights(self.ui_sim_3d_scene, intensity=30.)

        with ui.row().classes('w-full items-center gap-2') as self._sim_controls_row:
            self._sim_play_btn = ui.button(
                icon='play_arrow', on_click=self._toggle_sim_playback
            ).props('flat dense round')
            self._sim_frame_slider = ui.slider(
                min=0, max=1, step=1, value=0,
                on_change=self._on_sim_frame_slider
            ).classes('flex-1')
            self._sim_frame_label = ui.label('0 / 0').classes('text-sm min-w-[80px] text-right')
        self._sim_controls_row.set_visibility(False)

        self._sim_frame_objs = []
        self._sim_frame_glb_names = []
        self._sim_current_frame = 0
    ...
    def _sync_update_state(self):
        ...
        # Update cm labels with current body/design values
        self._update_all_cm_labels()
    ...
    async def run_dynamic_sim(self):
        """Run dynamic simulation with SMPL-X body, then show interactive 3D animation"""
        if not self.pattern_state.svg_filename:
            ui.notify('Current garment is empty. Choose a design first!',
                      type='warning', close_button=True)
            return

        self._stop_sim_animation()
        self.ui_sim_status.set_text('Simulating...')
        self.spin_dialog.open()
        ...
        try:
            ...
            result = await self.loop.run_in_executor(
                ...
                lambda: self.pattern_state.drape_3d_dynamic(
                    self.smplx_pose_idx,
                    self.smplx_beta_params
                )
            )
            out_dir, glb_filename, body_glb_filename, frame_glb_names = result

            self.ui_sim_status.set_text('Preparing 3D frames...')

            # Copy frame GLBs to static serving dir
            self._sim_frame_glb_names = []
            for fname in frame_glb_names:
                src = Path(out_dir) / fname
                dst_name = f'sim_{self.pattern_state.id}_{fname}'
                if src.exists():
                    shutil.copy2(src, self.local_path_3d / dst_name)
                    self._sim_frame_glb_names.append(dst_name)

            self.spin_dialog.close()

            self.ui_sim_status.set_text('Loading 3D frames...')
            await self._load_sim_animation()

            self.ui_sim_status.set_text('Complete')
            ...
            print(e)
            self.spin_dialog.close()
            self.ui_sim_status.set_text('Failed')
            ui.notify(
                f'Dynamic simulation failed: {e}',
                type='negative', close_button=True, position='center'
            )

    async def _load_sim_animation(self):
        """Pre-load all frame GLBs into the scene, then start playback"""
        for obj in self._sim_frame_objs:
            try:
                obj.delete()
            except Exception:
                pass
        ...
        if not self._sim_frame_glb_names:
            return

        self._sim_total_frames = len(self._sim_frame_glb_names)
        self._sim_current_frame = 0

        with self.ui_sim_3d_scene:
            for i, fname in enumerate(self._sim_frame_glb_names):
                obj = self.ui_sim_3d_scene.gltf(
                    f'geo/{fname}'
                ).scale(0.01).rotate(np.pi / 2, 0., 0.)
                obj.visible(i == 0)
                self._sim_frame_objs.append(obj)

        self._sim_frame_slider._props['max'] = max(self._sim_total_frames - 1, 1)
        self._sim_frame_slider.set_value(0)
        self._sim_frame_slider.update()
        self._sim_frame_label.set_text(f'1 / {self._sim_total_frames}')
        self._sim_controls_row.set_visibility(True)

        self._start_sim_playback()

    def _show_sim_frame(self, idx):
        """Toggle visibility: hide current frame, show requested frame"""
        if not self._sim_frame_objs:
            return
        idx = max(0, min(idx, self._sim_total_frames - 1))
        for i, obj in enumerate(self._sim_frame_objs):
            obj.visible(i == idx)
        self._sim_current_frame = idx
        self._sim_frame_label.set_text(f'{idx + 1} / {self._sim_total_frames}')

    def _toggle_sim_playback(self):
        if self._sim_playing:
            self._stop_sim_playback()
        else:
            self._start_sim_playback()

    def _start_sim_playback(self):
        if self._sim_total_frames < 2:
            return
        self._sim_playing = True
        self._sim_play_btn.props('icon=pause')
        self._sim_play_btn.update()
        if self._sim_timer is None:
            self._sim_timer = ui.timer(0.05, self._sim_timer_tick)
        else:
            self._sim_timer.activate()

    def _stop_sim_playback(self):
        self._sim_playing = False
        self._sim_play_btn.props('icon=play_arrow')
        self._sim_play_btn.update()
        if self._sim_timer is not None:
            self._sim_timer.deactivate()

    def _stop_sim_animation(self):
        self._stop_sim_playback()
        for obj in self._sim_frame_objs:
            try:
                obj.delete()
    ...
    def _sim_timer_tick(self):
        if not self._sim_playing or self._sim_total_frames < 2:
            return
        next_frame = (self._sim_current_frame + 1) % self._sim_total_frames
        self._show_sim_frame(next_frame)
        self._sim_frame_slider.set_value(next_frame)

    def _on_sim_frame_slider(self, e):
        self._show_sim_frame(int(e.value))
